                        ...
                        <form action="{{ route('units.update', $unit->id) }}" method="post"
                            ...
                            <div class="row gutters">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="kitchen_Cabinets" class="form-control SlectBox"
                                            ...
                                            <option
                                                ...
                                                نعم</option>
                                            <option value="no"{{ $unit->kitchen_Cabinets == 'no' ? 'selected' : '' }}>
                                                لا</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                ...
                                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-12">
                                    <div class="form-group">
                                        ...
                                        <select name="type" class="form-control SlectBox"
                                            ...
                                            <option value="فيلا"{{ $unit->type == 'فيلا' ? 'selected' : '' }}>فيلا
                                            </option>
                                            <option value="شقة"{{ $unit->type == 'شقة' ? 'selected' : '' }}>شقة
                                            </option>
                                            <option
                                                value="شقة ثنائية الدور"{{ $unit->type == 'شقة ثناشبة الدور' ? 'selected' : '' }}>
                                                شقة ثنائية الدور</option>
                                            <option value="شقة صغيرة"{{ $unit->type == 'شقة صغيرة' ? 'selected' : '' }}>
                                                شقة صغيرة</option>
                                            <option value="ملحق"{{ $unit->type == 'ملحق' ? 'selected' : '' }}>ملحق
                                            </option>
                                            <option value="مجل تجاري"{{ $unit->type == 'محل تجاري' ? 'selected' : '' }}>
                                                محل تجاري</option>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="size"
                                            value="{{ $unit->size }}"required>
                                    </div>
                                    <div class="form-group">
                                        <label class="active" for="sTate">عدد وحدات التكييف</label>
                                        <input type="number" class="form-control" name="condition_units"
                                            value="{{ $unit->condition_units }}"required>

                                    </div>
                                    <div class="form-group">
                                        <label class="active" for="fullName"> تفاصيل </label>
                                        <input type="text" class="form-control" id="details" name="details"
                                            value="{{ $unit->details }}"required>
                                    </div>
                                ...
                                <div class="col-xl-4 col-lg-4 col-md-4 col-sm-4 col-12">
                                    ...
                                    <div class="form-group">
                                        <label class="active" for="inputName" class="control-label">حالة الأثاث </label>
                                        <select name="furnished_mode" class="form-control SlectBox"
                                            onclick="console.log($(this).val())"
                                            onchange="console.log('change is firing')"required>
                                            <!--placeholder-->
                                            <option value="" selected disabled>حدد الحالة</option>
                                            <option
                                                value="unfurnished"{{ $unit->furnished_mode == 'unfurnished' ? 'selected' : '' }}>
                                                غير مؤثثة</option>
                                            <option
                                                value="newfn"{{ $unit->furnished_mode == 'newfn' ? 'selected' : '' }}>
                                                مؤثثة جديد</option>
                                            <option
                                                value="usedfn"{{ $unit->furnished_mode == 'usedfn' ? 'selected' : '' }}>
                                                مؤثثة مستعمل</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label class="active" for="inputName" class="control-label">نوع التكييف</label>
                                        <select name="condition_type" class="form-control SlectBox"
                                            onclick="console.log($(this).val())"
                                            onchange="console.log('change is firing')"required>
                                            <!--placeholder-->
                                            <option value="" selected disabled>حدد النوع</option>
